...
router.get('/', async (req, res) => {
    try {
        ...
        if (year_of_study)
            query.year_of_study = Number(year_of_study);
        if (branch_code)
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const db = (0, mongo_1.getDb)();
        const result = await db.collection('students').updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { ...req.body, updated_at: new Date() } });
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const student = await db.collection('students').findOne({ _id: new mongodb_1.ObjectId(id) });
        if (!student) {
            return res.status(404).json({ error: 'Student not found' });
        }
        // Delete the student record
        const deleteStudent = await db.collection('students').deleteOne({ _id: new mongodb_1.ObjectId(id) });
        // Delete all attendance records for this student
        let attendanceDeleted = 0;
        try {
            const delAttendance = await db.collection('attendance').deleteMany({ student_id: new mongodb_1.ObjectId(id) });
        ...
        let marksDeleted = 0;
        try {
            const delMarks = await db.collection('marks').deleteMany({ student_id: new mongodb_1.ObjectId(id) });
        ...
        try {
            if (student.profile_id) {
                ...
                if (mongodb_1.ObjectId.isValid(student.profile_id)) {
                    const del = await db.collection('profiles').deleteOne({ _id: new mongodb_1.ObjectId(student.profile_id) });
                    profileDeleted = del.deletedCount ?? 0;
                }
                else {
                    // fallback: try deleting by email if profile_id isn't an ObjectId
                    const del = await db.collection('profiles').deleteOne({ _id: student.profile_id });
                    profileDeleted = del.deletedCount ?? 0;
                }
            }
        }
